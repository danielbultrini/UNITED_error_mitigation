    ...
    for p = 16
        for seed = 0:30
           for nlsp = 1:3
               ...
               disp(nlsp)
               disp(p)
               seedsel = num2str(seed);
               Qsel = num2str(Q);
               nlsp_str = num2str(nlsp);
               p_sel = num2str(p);
               VD_CDR_data_exe(Qsel, p_sel, seedsel, N, Nts, max_copies, nlsp_str, restart,'0','0','./results/')
           end
        end
    end
end
...
%            nlsp_str = num2str(nlsp);
%            disp(nlsp_str)
%            VD_CDR_data_exe(Q, p, seed, N, Nts, max_copies, nlsp_str, restart,'0','0','')
%        end
%     end
% end
% toc
