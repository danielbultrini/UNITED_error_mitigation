...
function [obsn, prob0_array, prob0p_array,rhon,circc] = obs(circ, Q, max_copies, f,ii)
    [rhon,circc] = frhon_eff(circ,max_copies,Q,f); % conpoute \rho^n (Q qubits required) 
    obsn = obs_1q_noisy_eff(rhon{1},Q,ii);
    [prob0_array, prob0p_array] = prob_VD_eff(rhon,max_copies,Q,ii);
end

function [prob0_array,prob0p_array] = prob_VD_eff(rhon,max_copies,Q,ii)
...
for n = 2:max_copies
    ...
    a = ftr_rhon_obs(rhon{n},Q,ii);%compute Tr(\rho^n Z_1)
...
%function [obs] = obs_1q_noisy_eff_sigz(rho,Q)
%    sigz = diag([1 -1]);
%    rho = reshape(rho,[2 2^(Q-1) 2 2^(Q-1)]);
%    obs = real(ncon({rho,sigz},{[2 1 3 1],[3 2]}));
%end

function [obs] = obs_1q_noisy_eff(rho,Q,ii)
    sigx = [[0 1];[1 0]];
...
function [rhon,circc]  = frhon_eff(circ,n_max,Q,f)
