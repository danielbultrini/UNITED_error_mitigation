...
# %%
everything_full = pd.read_pickle(base_folder + "Swap_noise_complete_3nlspHalf.pkl")
# %%
everything_full = everything
everything_full.to_pickle(base_folder + "Swap_noise_complete_3nlspHalf.pkl")
# %%
...
t_swap.to_pickle(base_folder + "SWAP_DATA_v2_3nlsp_half.pkl")
...
a = (
    t_swap.query(
        'description == "SWAP_NOISE(half)" & res_type == "abs_error" & copies == 1 & nlsp == 1 & abs_error !=  0  & budget != 0   & type.str.startswith("ZNE")'
    )
    .copy()
...
b = t_swap.query(
    '  res_type == "abs_error" & copies == 4 &  nlsp == 1 & abs_error != 0  & budget != 0  & type.str.startswith("VD")'
).copy()
c = t_swap.query(
    ' description == "NORMAL(half)" &res_type == "abs_error" & copies == 4 &  nlsp == 1 & abs_error != 0  & budget != 0  & type.str.startswith("vnCG")'
).copy()
d = t_swap.query(
    ' description == "NORMAL(half)" &res_type == "abs_error" & copies == 4 & nlsp == 1 & abs_error !=  0  & budget != 0   & type.str.startswith("vnCD")'
).copy()
e = t_swap.query(
    ' description == "NORMAL(half)" &res_type == "abs_error" & copies == 5 &  nlsp == 1 & abs_error != 0  & budget == 10**10 & budget != 0  & type.str.startswith("vnCG")'
).copy()
f = t_swap.query(
    ' description == "NORMAL(half)" &res_type == "abs_error" & copies == 5 & nlsp == 1 & abs_error !=  0  & budget == 10**9 & budget != 0   & type.str.startswith("vnCD")'
).copy()


b_ = t_swap.query(
    ' description == "SWAP_NOISE(half)" & res_type =="abs_error" & copies == 3 &  nlsp == 1 & abs_error != 0  & budget != 0  & type.str.startswith("VD")'
).copy()
c_ = t_swap.query(
    ' description == "SWAP_NOISE(half)" & res_type =="abs_error" & copies == 3 &  nlsp == 1 & abs_error != 0  & budget != 0  & type.str.startswith("vnCG")'
).copy()
d_ = t_swap.query(
    ' description == "SWAP_NOISE(half)" & res_type =="abs_error" & copies == 3 & nlsp == 1 & abs_error !=  0  & budget != 0   & type.str.startswith("vnCD")'
).copy()
e_ = t_swap.query(
    ' description == "SWAP_NOISE(half)" & res_type =="abs_error" & copies == 5 &  nlsp == 1 & abs_error != 0  & budget == 10**10 & budget != 0  & type.str.startswith("vnCG")'
).copy()
f_ = t_swap.query(
    ' description == "SWAP_NOISE(half)" & res_type =="abs_error" & copies == 5 & nlsp == 1 & abs_error !=  0  & budget == 10**9 & budget != 0   & type.str.startswith("vnCD")'
).copy()

a_["description"] = "NORMAL(half)"
...
c_["shots"] = c_["shots"] // 10
d_["shots"] = d_["shots"] // 10
e_["budget"] = e_["budget"] // 10
f_["shots"] = f_["shots"] // 10
e_["shots"] = 2 * e_["shots"]
f_["shots"] = 2 * f_["shots"]

selection = pd.concat([a, b, c, d, o, e, f, a_, o_, c_, d_, e_, f_])
...
g.map_dataframe(
    ...
    yscale="log",
    xscale="log",
    xlim=[10**5, 10**10 + 10**8],
    xticks=[10**5, 10**6, 10**7, 10**8, 10**9, 10**10],
    xticklabels=[r"$10^5$", r"$10^6$", r"$10^7$", r"$10^8$", r"$10^9$", r"$10^{10}$"],
)
g.add_legend()
g.set_axis_labels("Budget", r"Mean $\langle \sigma^1_z \rangle$ absolute error")
g.tight_layout()  # g.fig.delaxes(g.axes[-1, -1])
# %%
# %%
#### with inf
o = t_swap.query(
...
g.map_dataframe(
    ...
    yscale="log",
    xscale="log",
    xlim=[10**5, 10**11 + 10**8],
    xticks=[10**5, 10**6, 10**7, 10**8, 10**9, 10**10, 10**11],
    xticklabels=[
        r"$10^6$",
        r"$10^6$",
        r"$10^7$",
...
with open("dataframe_MAX_swap_noise_complete.txt", "w") as csv_file:
    ...
    for thing in selection.type.unique():
        print(thing)
        for nlsp in [1]:
            for description in ["SWAP_NOISE(half)", "NORMAL(half)"]:
                for budget in [
                    10**5,
                    10**6,
                    10**7,
                    ...
                    for q, g in zip([4], [1]):
                        for result_type in ["abs_error"]:
                            x = selection.query(
                                f"     description == '{description}' & res_type == '{result_type}' &budget == {budget} & type == '{thing}' & nlsp == {nlsp} & Qubits == {q} "
                            )["abs_error"].max()
                            frame = selection.query(
                                f" description == '{description}' & budget == {budget} & type == '{thing}' & nlsp == {nlsp} & Qubits == {q} "
                            )
                            try:
                                ...
                                cop = "no"
                            csv_writer.writerow(
                                [thing, budget, q, g, x, 3, desc, result_type]
                            )
# %%
